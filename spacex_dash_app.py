...
def get_pie_chart(entered_site):
    ...
    if entered_site == "ALL":
        ...
        data = (
            filtered_df.groupby(["Launch Site"]).size().reset_index(name="class count")
        )
        data["pie chart names"] = data["Launch Site"]
        title = "Success vs. Failed Launches for All Sites"
        fig = px.pie(data, values="class count", names="pie chart names", title=title)
        return fig
    else:
        # return the outcomes piechart for a selected site
        data = filtered_df[filtered_df["Launch Site"] == entered_site]
        data = data.groupby(["class"]).size().reset_index(name="class count")
        data["pie chart names"] = data["class"]
